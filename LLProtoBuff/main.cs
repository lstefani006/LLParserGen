...
namespace LLProtoBuff
{
	...
	class Program
	{
		...
		private static void Help()
		{
			...
			Console.WriteLine("-n:<Namespace>             forza il namespace ignorando le direttive dentro il file .proto");
			Console.WriteLine();
			...
			try
			{
				...
				for (i = 0; i < args.Length; ++i)
				{
					...
					default:
						...
						{
							pkgName = args[i].Substring("-n:".Length);
							continue;
						}
						else if (args[i] == "-o")
						{
					...
					using (xw)
					{
						...
						if (cs)
						{
							...
							else
							{
								...
								GenCS_PB(dg, tw, csFlags, pkgName);
							}
						}
		...
		private static void GenCS_PB(DeclList dg, U.CsStreamWriter tw, CsFlags csFlags, string pkgName)
		{
		...
		private static void GenHPP(DeclList dg, U.CsStreamWriter tw, string fileHpp, CppFlags cppFlags, string dll)
		{
				...
				(cppFlags & CppFlags.ClientPbCall2) == CppFlags.ClientPbCall2)
			{
				foreach (var s in from v in dg where v.IsService select (ServiceDecl)v)
				{
					...
					tw.WriteLine("{");
					tw.WriteLine("public:");
					tw.WriteLine($"static const char ServiceName[];");
					tw.WriteLine($"{s.Name.strRead}Client(const U::ProtoClientInit &m) : {protoClient}(m) {{}}");
					...
					foreach (var f in s.Fun)
						tw.WriteLine($"void {f.Name.strRead}({f.Req.strRead} &req, {f.Res.strRead} &res, bool rest = false);");
					tw.WriteLine("};");
				}
			}
		...
		private static void GenCPP(DeclList dg, U.CsStreamWriter tw, string fileHpp, CppFlags cppFlags)
		{
				...
				(cppFlags & CppFlags.ClientPbCall2) == CppFlags.ClientPbCall2)
			{
				foreach (var s in from v in dg where v.IsService select (ServiceDecl)v)
				{
					tw.WriteLine($"const char {s.Name.strRead}Client::ServiceName[] = \"{s.Name.strRead}\";");
					foreach (var f in s.Fun)
						tw.WriteLine($"void {s.Name.strRead}Client::{f.Name.strRead}({f.Req.strRead} &req, {f.Res.strRead} &res, bool rest) {{ Exec(ServiceName, \"{f.Name.strRead}\", req, res, rest); }};");
					tw.WriteLine();
				}
			}
